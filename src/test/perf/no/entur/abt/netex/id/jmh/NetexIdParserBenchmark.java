...
public class NetexIdParserBenchmark {
    ...
    @Benchmark
    public long typeNetexIdUtilsLegacy() {
        long count = 0;
        for (String id : IDS) {
            count += LegacyNetexIdUtils.getType(id).length();
        }
        return count;
    }
    ...
    @Benchmark
    public long codespaceNetexIdUtilsLegacy() {
        long count = 0;
        for (String id : IDS) {
            count += LegacyNetexIdUtils.getCodespace(id).length();
        }
        return count;
    }
